...
impl Number {
    ...
    /// Constructs a `Number` form an usize value.
    pub fn from_usize(x: usize) -> Self {
        Number(BigRational::from_i128(x as i128).unwrap())
    }
    /// Constructs a `Number` form an isize value.
    ...
    pub fn save_div(&self, den: &Number) -> Option<Number> {
        if den == &Number::int(0) {
            None
        } else {
            Some(self / den)
